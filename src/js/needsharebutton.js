	  ...
	  root.share = {
	  	...
	  	'twitter' : function() {
			http://twitter.com/share?text=text goes here&url=http://url goes here&
			  var url = root.options.protocol + 'www.twitter.com/share?';
        ...
        root.popup(url);
	  	...
	  	'facebook' : function() {
			  var url = root.options.protocol + 'www.facebook.com/dialog/feed?';
			  url += 'app_id=464293104431558';
			  url+='&display=popup';
			  url+='&link='+encodeURIComponent(root.options.url);
			  url+='&redirect_uri='+window.location.origin+window.location.pathname+"?id=getout";
